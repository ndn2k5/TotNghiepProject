...
- **Global scope**: `D:/PROJEct/AI MODELS/TotNghiepProject/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/PROJEct/AI MODELS/TotNghiepProject/.claude/notes/`)
...
2. Glob `D:/PROJEct/AI MODELS/TotNghiepProject/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/PROJEct/AI MODELS/TotNghiepProject/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/PROJEct/AI MODELS/TotNghiepProject/.claude/notes/` — works in any directory
